...
class Profile(models.Model):
	...
	def send_activation_email(self):
		...
		if not self.activated:
			...
			message = path_		#f"Activate your account: {path_}"
			recipient_list = [self.user.email]
			html_message = '<b>Activate your account:' + path_ + '</b>'
			print(html_message)
			# sent_mail = send_mail(
			# 		subject,
			# 		message,
...
def post_save_user_receiver(sender, instance, created, *args, **kwagrs):
	if created:
		profile, is_created = Profile.objects.get_or_create(user=instance)
		default_user_profile = Profile.objects.get_or_create(user__id=1)[0] #user__username=
		default_user_profile.followers.add(instance)	#add instance follower to admin (user__id=1)
		# default_user_profile.followers.remove(instance)
		# default_user_profile.save()
		profile.followers.add(default_user_profile.user)	#add admin follower to instance
		profile.followers.add(2)							#add user_id=2 follower to instance
